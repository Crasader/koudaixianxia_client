...
void Strengthen::initBaseProperty(float flt){
...
void Strengthen::analyzeData(std::string data){
    ...
    if (reader.parse(data.c_str(), root)) {
        ...
        for (unsigned int i=0; i< teamInfo.size(); i++) {
        ...
        if (reader.parse(msg->data, root)) {
            if (root["result"].asBool()) {
                ...
                sList->setNewProperties(djmoney, hpmoney, attmoney, addhp, addatt, luck, newfighting, newlevellimit, newcolor);
                    this->scheduleOnce(schedule_selector(Strengthen::initBaseProperty), 0.01f);
                }
            }
        }
    }
...
void Strengthen::finishedInit(CCObject *obj){
    ...
    if (msg) {
        ...
        json_strs= msg->data;
        this->scheduleOnce(schedule_selector(Strengthen::setStentgt), 0.001f);
    }
    
}
void Strengthen::setStentgt(float flt){
