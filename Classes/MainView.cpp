...
void MainView::LayerEaseIn(CCNode * Layersender,CCMenu * menu){
    if (Layersender->getPosition().x>-(size.width/2)) {
        CCMoveTo * move=CCMoveTo::create(0.3f, ccp(0, 0));
        ...
        CCMoveTo * menu_Move=CCMoveTo::create(0.3f, ccp(size.width, menu->getPosition().y));
        ...
        CCMoveTo * move=CCMoveTo::create(0.3f, ccp(-size.width, 0));
        CCEaseIn * easein=CCEaseIn::create(move, 1);
...
void MainView::receiveWelfareMsg(CCObject *pObj){
    ...
    this->scheduleOnce(schedule_selector(MainView::getWelfareMsg), 0.001f);
    CCNotificationCenter::sharedNotificationCenter()->removeObserver(this, "welfare");
    
}
void MainView::getWelfareMsg(float flt){
