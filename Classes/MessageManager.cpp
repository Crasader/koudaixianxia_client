...
void MessageManager::sendReleaseRoleMessage(int petId){
    CSJson::FastWriter writer;
    CSJson::Value root;
    root["pid"]=GameData::shareGameData()->chaId;
    root["ppid"]=petId;
    std::string jsonstr=writer.write(root);
    Message *msg=g_pSocket->constructMessage(jsonstr, 1107);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(MessageManager::receiveReleaseRole), "1107", NULL);
    CCLog("%s",jsonstr.c_str());
    g_pSocket->sendMessage_(msg, false);
}
void MessageManager::receiveReleaseRole(CCObject *obj){
    CCAssert(obj, "null");
    CCNotificationCenter::sharedNotificationCenter()->removeObserver(this, "1107");
    CCNotificationCenter::sharedNotificationCenter()->postNotification("releaseRole",obj);
}
void MessageManager::sendPrepareForWar(int petId,int targetId){
    CSJson::FastWriter writer;
    CSJson::Value root;
    root["pid"]=GameData::shareGameData()->chaId;
    root["moveid"]=petId;
    ...
    std::string jsonstr=writer.write(root);
    Message *msg=(Message *)g_pSocket->constructMessage(jsonstr, 1105);
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(MessageManager::receiveTPrepareForWar), "1105", NULL);
    g_pSocket->sendMessage_(msg, false);
}
void MessageManager::receiveTPrepareForWar(CCObject *obj){
    CCAssert(obj, "null");
    CCNotificationCenter::sharedNotificationCenter()->removeObserver(this, "1105");
    CCNotificationCenter::sharedNotificationCenter()->postNotification("PrepareForWar");
}
