...
void LoginScene::socketError(cocos2d::CCObject *obj){
    CCLOG("Net Erorr");
}
    ...
    if(msg){
        ...
        if (reader.parse(msg->data, root)) {
            if (root["result"].asBool()) {
                ...
                msgbox->CMessageBox("´í  Îó", "³ö´íÁË£¡");
            }
        }
    }
}
void LoginScene::registerMenuCallBack(float flt){
...
void LoginScene::LayerEaseIn(CCNode *Layersender){
    if (Layersender->getPosition().x<(-g_cSize.width/2)) {
        CCMoveTo *move=CCMoveTo::create(0.3f, ccp(-g_cSize.width, 0));
        ...
        CCMoveTo *menu_Move=CCMoveTo::create(0.3f, ccp(-g_cSize.width,g_cSize.height/2-100));
        ...
        CCScaleTo *scale = CCScaleTo::create(0.3f, 1,1);
        CCFadeTo *fade = CCFadeTo::create(0.3f, GLubyte(0));
        ...
        CCMoveTo *move=CCMoveTo::create(0.3f, CCPointZero);
        CCEaseIn *easein=CCEaseIn::create(move, 1);
        Layersender->runAction(easein);
            CCScaleTo *scale= CCScaleTo::create(0.3f, 1, 0.95f);
        CCFadeTo *fade = CCFadeTo::create(0.3f, GLubyte(150));
        ((MainView *)m_pLoadLayer)->color->runAction(fade);
        m_pLoadLayer->runAction(scale);
...
void LoginScene::MoveAndEase(cocos2d::CCPoint pos,CCNode * sender){
    CCMoveTo *menu_Move=CCMoveTo::create(0.3f, pos);
    CCEaseIn *menu_EaseIn=CCEaseIn::create(menu_Move, 1);
