...
bool WelfareLayer::init(){
    ...
    CCNotificationCenter::sharedNotificationCenter()->addObserver(this, callfuncO_selector(WelfareLayer::receiveNoitce), "loadingremove", NULL);
    ...
    CCLabelTTF *coinLabel=CCLabelTTF::create(coin, SELF_TTF_ONE, 60, CCSizeMake(200, 70), kCCTextAlignmentLeft);
    coinLabel->setAnchorPoint(ccp(0, 0));
    coinLabel->setPosition(ccp(winSize.width/2-30, 880));
    coinLabel->setColor(fontColor);
    this->addChild(coinLabel,1);
    CCLabelTTF *goldLabel=CCLabelTTF::create(gold, SELF_TTF_ONE, 60, CCSizeMake(200,70), kCCTextAlignmentLeft);
    goldLabel->setAnchorPoint(ccp(0, 0));
    goldLabel->setPosition(ccp(winSize.width*3/4+60, 880));
...
Welfare *wf=(Welfare *)m_welfareArr->objectAtIndex(idx);
    ...
    CCLabelTTF *coinCount=CCLabelTTF::create(coin, SELF_TTF_ONE, 60, CCSizeMake(300, 70), kCCTextAlignmentLeft);
    ...
    CCLabelTTF *descriptionslabel=CCLabelTTF::create(wf->descriptions.c_str(), SELF_TTF_ONE, 60, CCSizeMake(800, 70), kCCTextAlignmentLeft);
...
void WelfareLayer::ccTouchesBegan(CCSet* set, CCEvent* event){
    ...
    for (; sel!=set->end(); sel++) {
        ...
        if (touchNum==2) {
//            CCLog("ccTouchesBegan %d",pTouch->getID());
//            if (pTouch->getID()==0) {
//                distance=pTouch->getLocation().x-this->boundingBox().origin.x;
//                CCLog("distance   %f",distance);
//            }
...
void WelfareLayer::ccTouchesMoved(CCSet* set, CCEvent* event){
    ...
    for (; sel!=set->end(); sel++) {
        ...
        if (touchNum>1) {
            ...
            if (fabs(m_beginPoint.y-pTouch->getLocation().y)<20) {
...
void WelfareLayer::setData(float flt){
...
void WelfareLayer::receiveNoitce(cocos2d::CCObject *obj){
    if (obj==this) {
        this->scheduleOnce(schedule_selector(WelfareLayer::replaceScene), 0);
    }
}
void WelfareLayer::replaceScene(float dt){
...
void WelfareLayer::easeOut(CCNode *sender){
    ...
    if (cell->getPosition().x>600) {
        ...
        CCCallFunc *func=CCCallFuncN::create(this, callfuncN_selector(WelfareLayer::easeOutCallBack));
        ...
        lqcgAni->setScale(2);
        func=CCCallFunc::create(this, callfunc_selector(WelfareLayer::lqcgCallBack));
    ...
    CCMoveTo *moveto=CCMoveTo::create(0.3f, endedPos);
...
void WelfareLayer::easeOutCallBack(CCNode *pSender){
    CCTableViewCell *cell=(CCTableViewCell *)pSender;
...
void WelfareLayer::reloadInfo(CCObject *obj){
    ...
    this->scheduleOnce(schedule_selector(WelfareLayer::setData), 0.01f);
