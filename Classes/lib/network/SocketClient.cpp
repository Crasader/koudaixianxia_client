...
//    str.append(msg->HEAD0);
   ...
   // CCLog("%s",data);
    ...
    //msg->data = data;
	CCLog("[Send]commandId:%d Length:%d Msg:%s",bytesToInt(msg->commandId),data.size(),data.c_str());
	return msg;
}
...
bool SocketClient::connectServer()
{
	...
	CCLog("Connect:%s:%d",m_host.c_str(),wPort);
	if( m_hSocket != -1){
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
		closesocket(m_hSocket);
#else
...
//	}
	...
	if (iErrorCode==-1)
	{
        CCNotificationCenter::sharedNotificationCenter()->postNotification("socketError");
...
//							message->seqNum = recvBuff->getInt();
                            ...
                            message->data = tmp;
                            CCLog("%s",tmp);
                            if (bytesToInt(message->commandId)==1104) {
//                                allPetInfo1101::allpetInfoResponse response;
//                                response.ParseFromString(message->data);
//                                CCLog("%d",response.pfinfo_size());
                            }
...
//                                CCNotificationCenter::sharedNotificationCenter()->postNotification("1001");
                                CCLog("[Rece]commandId:%d Length:%d Msg:%s",bytesToInt(message->commandId),sizeof(This->m_receivedMessageQueue),message->data);
                                GameData::shareGameData()->msg_dic->setObject(message, bytesToInt(message->commandId));
