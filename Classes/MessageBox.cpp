...
class MessageBox *MessageBox::create(){
    class MessageBox* pRet = new MessageBox();
    if (pRet && pRet->init())
    {
        pRet->autorelease();
    }
...
void MessageBox::goseAction(){
    CCScaleTo *actScale=CCScaleTo::create(0.1f, 1);
...
void MessageBox::ccTouchMoved(CCTouch* touch, CCEvent* event){
    if (m_bSureBtnFlag) {
        ...
        if (touch->getLocation().x-m_dGap<140) {
            ...
            CCScaleTo *aScaleTo=CCScaleTo::create(0.1f, 0);
