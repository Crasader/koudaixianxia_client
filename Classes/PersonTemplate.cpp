...
bool Person::init(){
    ...
    target=NULL;
    skill=NULL;
    setStaticProperties(3000, 1000, 2, 2, 2, 2, 2, 0.4f, 0.4f, 0.4f);
...
void Person::animateCallBack(CCNode *sender){
...
void Person::reduceBloodCallBack(CCNode *sender){
    CCSprite *sp=(CCSprite *)sender;
    sp->removeFromParentAndCleanup(true);
}
void Person::changeBlood(int changeBlood){
    ...
    if(cur_hp+changeBlood>0){
		...
		CCScaleTo * scale= CCScaleTo::create(fabs((float)changeBlood)/(float)m_hp+0.1, ((float)cur_hp/(float)m_hp)*0.5, 0.5);
        ...
        reduceBlood->runAction(fadeout);
        m_role->addChild(reduceBlood,1);
        CCScaleTo * scale= CCScaleTo::create(changeBlood/m_hp, 0, 0.5f);
...
//    }
    CCDelayTime *delay0= CCDelayTime::create(0.416f);
    CCCallFunc *func;
    CCSequence *seq;
...
void FriendlyRole::traceTarge(){
    ...
    if (distance<=target->m_range) {
      ...
      //  isAttacking=false;
        ...
        CCCallFuncND *func=CCCallFuncND::create(this, callfuncND_selector(FriendlyRole::callBackStand),NULL);

        CCSequence *seq=CCSequence::create(move,func,NULL);
        this->runAction(seq);
    }
}
void FriendlyRole::setZs(float flt){
...
void FriendlyRole::touchEvent(cocos2d::CCTouch *touch,bool touch_flag){
    ...
    if (touch_flag) {//攻击
        ...
        for (unsigned int i=0; i<bl->appearArr->count(); i++) {
...
void FriendlyRole::enterAttackOrgin(float flt){
    return;
...
void FriendlyRole::moveToSelectTarget(float flt){
...
void FriendlyRole::enterAttackRegion(float flt){
    if (isAttacking) {
        return;
    }
...
void FriendlyRole::callBackAttack(CCNode *sPend,void* vd){
...
void FriendlyRole::callBackStand(CCNode *sPend,void* vd){
    beatBackFlag=true;
    isRunning=false;
    isAttacking=false;
...
void FriendlyRole::attackCallBack(){
    if (!isRunning) {
        if (target->m_hp>0 &&!target->isDead) {
            motionType(STAND, aniDir, ARMATURE_ACTION_DATA0);
            CCDelayTime *dalay0= CCDelayTime::create(0.42f);
...
void FriendlyRole::judgePos(){
    ...
    if (this->getPosition().x-this->target->getPosition().x>0&&this->getPosition().x-this->target->getPosition().x<NEAR_MINATTACKRANGE) {
        ...
        if (this->target->getPosition().y>this->getPosition().y) {
            ...
            CCCallFuncND *func=CCCallFuncND::create(this, callfuncND_selector(FriendlyRole::callBackAttack),NULL);
...
void CureRole::attackCallBack(){
    ...
    CCDelayTime *delay1=CCDelayTime::create(0.45f);
    isCure=true;
...
void EnemyRole::enterAttackRegion(float flt){
...
void EnemyRole::PinballDis(float flt){
...
void EnemyRole::callBackStand(CCNode *sender, void *num){
    isRunning=false;
...
//    this->m_role->getAnimation()->stop();//骨骼动画
  //  m_role->stopActionByTag(00000);
    ...
    CCFadeIn *fadein=CCFadeIn::create(0.15f);
...
void EnemyRole::changeDir(float flt){
...
void EnemyRole::judgeSelfPos(){
    ...
    if ( this->getPosition().x-this->target->getPosition().x>0&&this->getPosition().x-this->target->getPosition().x<=m_range&&!ismoveing) {
        ...
        if (this->target->getPosition().y>this->getPosition().y) {
            ...
            CCCallFuncND *func=CCCallFuncND::create(this, callfuncND_selector(EnemyRole::runCallBack),NULL);
            CCSequence *seq=CCSequence::create(moveToPos,func,NULL);
            seq->setTag(1001);
            this->runAction(seq);
        }else{
...
void EnemyRole::runCallBack(CCNode *sender, void *num){
    isRunning=false;
    float distance= ccpDistance(this->getPosition(), target->getPosition());
    if (distance<=m_range) {
        judgeSelfPos();
...
void EnemyRole::beatBack(){
  ...
  //  getHostility();
   ...
   // getHostility();
    for (unsigned int i=0; i<GameData::shareGameData()->roleArr->count(); i++) {
...
void EnemyRole::getHostility(float flt){
    for (unsigned int i=0; i<GameData::shareGameData()->roleArr->count(); i++) {
...
void EnemyRole::setZs(float flt){
    if (target) {
        if (this->getPosition().x>this->target->getPosition().x) {
            aniDir=2;
        }else{
...
void MasterRole::setZs(float flt){
    float z=-this->getPositionY()/1;
    this->setZOrder(z);
    if (aniDir==1) {
        skillbtns->setPosition(ccp(-150, 60));
...
void MasterRole::touchSchedul(float flt){
    if (fireBall==NULL) {
        this->stopAllActions();
        this->unscheduleAllSelectors();
        m_strongTouch=false;
...
void MasterRole::sendAttackJson(){
    ...
    root["skillId"]=1;
    for (unsigned int i=0;i<info0->attackeds->count();i++) {
...
void MasterRole::receiveAttackJson(float flt){
...
void MasterRole::enterAttackRegion(float flt){
    BattleLayer *bl=(BattleLayer *)this->getParent();
    for (unsigned int i=0; i<bl->appearArr->count(); i++) {
        EnemyRole *enemy=(EnemyRole *)bl->appearArr->objectAtIndex(i);
...
void MasterRole::enemyDead(CCObject *sender){
    ...
    if (enemy==target) {
        ...
        motionType(STAND, aniDir, ARMATURE_ACTION_DATA6);
        CCDelayTime *delay=CCDelayTime::create(1.5);
        CCCallFuncND *func=CCCallFuncND::create(this, callfuncND_selector(MasterRole::callBackStand),NULL);
        CCSequence *seq=CCSequence::create(delay,func,NULL);
        this->runAction(seq);
...
void MasterRole::moveFireball(float flt){
    if (!isRunning) {
...
//        this->runAction(seq1);
        CCDelayTime *delay=CCDelayTime::create(1.5);
        CCCallFuncND *func=CCCallFuncND::create(this, callfuncND_selector(MasterRole::callBackAttack),NULL);
        CCSequence *seq=CCSequence::create(delay,func,NULL);
        seq->setTag(302);
        this->runAction(seq);
        motionType(STAND, aniDir, ARMATURE_ACTION_DATA6);
...
void EnemyLongRangeBoss::runCallBack(){
    isRunning=false;
    float distance= ccpDistance(this->getPosition(), target->getPosition());
    if (distance<=500) {
...
void EnemyLongRangeBoss::enterAttackRegion(float flt){
    if (ccpDistance(target->getPosition(), this->getPosition())<=500) {
